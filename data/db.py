...
def add_user(user_id: int, member=False, premium=False, banned=False, mod=0,  premium_exp_date=None, last_command=" ",daily=None,xp=10,coins=0):
    if premium_exp_date is None:
        premium_exp_date = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    if daily is None:
    ...
    try: 
        ...
        with sqlite3.connect('data/users.db') as conn:
            cursor = conn.cursor()
            cursor.execute('''INSERT OR REPLACE INTO users (user_id, member, premium, banned, mod,  premium_exp_date, last_command, daily, xp, coins)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ? )''',
                        (user_id, member, premium, banned, mod,  premium_exp_date, last_command,daily,xp,coins))
            conn.commit()
